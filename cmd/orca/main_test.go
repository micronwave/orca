...
func TestDefaultConfigYAMLIncludesAdvancedAndLoads(t *testing.T) {
	...
	yaml := config.DefaultConfigYAML("go")
	if err := os.WriteFile(path, []byte(yaml), 0o644); err != nil {
		t.Fatalf("write config: %v", err)
	}
	cfg, err := config.Load(path)
	if err != nil {
		t.Fatalf("Load(DefaultConfigYAML(go)): %v", err)
	}
	...
	if !strings.Contains(yaml, "advanced:") {
		t.Fatal("DefaultConfigYAML(go) does not contain 'advanced:' block")
	}
}
...
func TestAutoInitCreatesOrcaDirAndConfig(t *testing.T) {
	parent := t.TempDir()
	...
	if _, err := os.Stat(filepath.Join(orcaDir, "events.log")); err != nil {
		t.Fatalf("events.log not created: %v", err)
	}
	if _, err := os.Stat(filepath.Join(orcaDir, "capsules")); err != nil {
		t.Fatalf("capsules dir not created: %v", err)
	}
}

func TestAutoInitIsIdempotent(t *testing.T) {
	orcaDir := seedOrcaDir(t, false)
...
func TestAutoInitWrittenConfigLoads(t *testing.T) {
	parent := t.TempDir()
	orcaDir := filepath.Join(parent, ".orca")
	...
	// The auto-generated config must be loadable (it will have gates because
	// the test process can walk up to find go.mod in the orca module root).
	_, err := configLoad(filepath.Join(orcaDir, "config.yaml"))
	if err != nil {
		t.Fatalf("config generated by autoInit did not load: %v", err)
	}
}
...
func TestIsattyReturnsFalseForRegularFile(t *testing.T) {
